...
class PdoMapper extends Mapper
{
    ...
    public function query($query, $cast = null)
    {
        // execute
        $result = $cast
            ? $this->_pdo->query($query, \PDO::FETCH_CLASS, $this->_models[$cast])
            : $this->_pdo->query($query, \PDO::FETCH_OBJ);
    ...
    {
        // prepare sql
        $sql = 'SELECT COUNT(*) FROM `' . $this->_prefix . $alias . '`';
    ...
    {
        // prepare sql
        $sql = 'SELECT * FROM `' . $this->_prefix . $alias . '`';
        ...
        // result set
        $rows = !empty($this->_models[$alias])
            ? $sth->fetchAll(\PDO::FETCH_CLASS, $this->_models[$alias])
    ...
    public function save($alias, &$entity)
    {
        ...
        if(empty($data['id']))
        {
            ...
            // prepare sql
            $sql = 'INSERT INTO `' . $this->_prefix . $alias . '`';
        ...
        else
        {
            ...
            // prepare sql
            $sql = 'UPDATE `' . $this->_prefix . $alias . '` SET ';
        ...
        if($result) {
            ...
            // has id
            $newId = $entity->id ?: $this->_pdo->lastInsertId();
            if($newId)
    ...
    public function drop($alias, $entity)
    {
        ...
        // prepare sql
        $sql = 'DELETE FROM `' . $this->_prefix . $alias . '`';
